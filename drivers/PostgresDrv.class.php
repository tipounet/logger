 ...
 * @version 1.0
 */
class PostgresDrv implements DriversInt {
	/**
	 * Instance de PDO
	 *
	 * @var \PDO
					...
					if (empty ( $host) === false) {
						if ($proprietes->containsKey ( 'username' ) === true) {
							...
							*/
							...
							/*
							 $this->pdo = new \PDO('pgsql:host='.
							 		$proprietes->get ('host').';port=5432;dbname='.
							 		$proprietes->get ('dbname').';user='.
							 		$proprietes->get ('username').';password='.$proprietes->get('passwd'));
							*/
...
insert;
		$requete = array (
				...
				'sev' => 'create table severites(idSeverite serial not null  primary key, nom varchar(50)  not null);',
				'logs' => 'create TABLE logs ( idLog serial not null primary key,
	...
	/**
	 * retourne la version de postgre SQL
	 */
	public function versionPgsql(){
		if ($this->pdo !== null){
